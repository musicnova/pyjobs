...
import collections
import pyjobs
from pyjobs.app.base import SCAN_AGENT, SCAN_PAGE, SCAN_TASK
import pytest
...
class Key(pyjobs.Record):
    value: int


class Value(pyjobs.Record, serializer='json'):
    amount: float
...
class test_App:
    ...
    def test_new_transport(self, broker_url, broker_consumer_url, *,
        ...
        by_url = patching('pyjobs.transport.by_url')
    ...
    def test_discover(self, *, app):
        ...
        app.conf.origin = 'pyjobs'
        fixup1 = Mock(name='fixup1', autospec=Fixup)
        fixup1.autodiscover_modules.return_value = ['d', 'e']
        app.fixups = [fixup1]
        with patch('pyjobs.app.base.venusian'):
    ...
    def test_discover_ignore(self, *, app):
        with patch('pyjobs.app.base.venusian') as venusian:
            app.conf.origin = 'pyjobs'
            app.conf.autodiscover = ['re', 'collections']
            app.discover(categories=['pyjobs.agent'], ignore=['re', 'pyjobs'])

            assert venusian.Scanner().scan.assert_has_calls([
                call(
                    re,
                    categories=('pyjobs.agent', ),
                    ignore=['re', 'pyjobs'],
    ...
    def test__on_autodiscovery_error(self, *, app):
        with patch('pyjobs.app.base.logger') as logger:
            try:
                raise KeyError('foo')
    ...
    def test_main(self, *, app):
        with patch('pyjobs.cli.pyjobs.cli') as cli:
    ...
    async def test_crontab(self, *, app):
        ...
        with patch('pyjobs.app.base.cron') as cron:
    ...
    def test_reply_consumer(self, *, app):
        with patch('pyjobs.app.base.ReplyConsumer') as ReplyConsumer:
...
class test_AppConfiguration:
    ...
    def test_conf__before_finalized(self, *, monkeypatch, app):
        app.finalized = False
        monkeypatch.setattr('pyjobs.app.base.STRICT', False)
        app.conf.id
        monkeypatch.setattr('pyjobs.app.base.STRICT', True)
        with pytest.raises(ImproperlyConfigured):
            app.conf.id
    ...
    def test_load_settings_from_source__no_attribute(self, *, app):
        with pytest.raises(AttributeError):
            app._load_settings_from_source('pyjobs.app.base.NOTEXIST')
        assert app._load_settings_from_source(
            'pyjobs.app.base.NOTEXIST', silent=True) == {}
